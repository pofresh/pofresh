...
Application._loadPluginComponents = function(componentsPath, opts) {
    ...
    const componentFiles = fs.readdirSync(componentsPath).filter(filename => JS_FILE_REGEX.test(filename));
...
Application._loadPluginEvents = function(eventsPath) {
    ...
    const eventFiles = fs.readdirSync(eventsPath).filter(filename => JS_FILE_REGEX.test(filename));
...
function bindEvents(EventClass, app) {
    ...
    const methodNames = Object.keys(eventMethods);
