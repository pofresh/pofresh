...
class MailStation extends EventEmitter {
    ...
    removeServer(id) {
        ...
        for (const type in this.serversMap) {
            const index = this.serversMap[type].indexOf(id);
            if (index >= 0) {
                this.serversMap[type].splice(index, 1);
            }
        }
    ...
    connect(tracer, serverId, cb) {
        ...
        mailbox.connect(tracer, err => {
            if (err) {
                ...
                this.emit('error', constants.RPC_ERROR.FAIL_CONNECT_SERVER, tracer, serverId, null, this.opts, cb);
                return;
            }
            mailbox.on('close', id => {
