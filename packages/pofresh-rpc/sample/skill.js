...
/// 计算技能伤害
SkillAction.calDamage = (equiper, equipObject, target, skill, addParas) => {
    const p = [];
    for (let i = 0; i < 17; i++) {
        p[i] = 1;
    }
...
/// 计算军师技能伤害
SkillAction.calAdviserDamage = (equiper, equipObject, target, skill, _addParas) => {
    const p = [];
    p[0] = equipObject;
    p[1] = target;
    p[2] = skill;
...
SkillAction.scopeLoop = (cx, cy, cs, cb, cells) => {
    if (!cells) {
...
SkillAction.initTileNodes = (cx, cy, cs) => {
    const map = SkillAction.wLayer.map;
    const node = new cc.Node();
    const cells = map.scopeCellsByTileXY(cx, cy, cs, true);
    const centerMapPos = map.tile2map(cx, cy);
    node.x = centerMapPos[0];
    node.y = centerMapPos[1];
    SkillAction.wLayer._bgEffNode.addChild(node);
    for (let i = 0; i < cells.length; i++) {
        const cellPos = map.tile2map(cells[i].cx, cells[i].cy);
        const tileNode = new TileNode(SkillAction.wLayer);
...
/// 寻找包裹路径的格子
SkillAction.findCellsAroundPath = path => {
    const cellsMap = {};
    for (let i = 0; i < path.length; i++) {
        ...
        const cells = SkillAction.wLayer.map.scopeCells(cell.cx, cell.cy, 1, false);
...
SkillAction.checkTargetBeDebuff = target => CONFLICT.checkByTask(target, 'beDebuff');

/// 检验技能影响类型
SkillAction.checkAffectType = (target, self, action) => {
    let ret;
...
SkillAction.getActionTarget = (target, action) => {
    let ret = null;
    switch (action.targetType) {
        ...
        case SKILL_DEF.TARGET_BUILDING:
            if (
                target.type === MAP_DEF.TYPE_BUILDING_PORT ||
                target.type === MAP_DEF.TYPE_BUILDING_GATE ||
                target.type === MAP_DEF.TYPE_BUILDING_CASTLE ||
                SkillAction.wLayer.workpieceMgr.checkWorkPiece(target)
            ) {
                ret = target;
            }
            break;
...
/// 绑定一次技能
SkillAction.bindSkillOnce = paras => {
    const target = paras[0]; //技能目标
    const _equiper = paras[1]; //技能装备的武将
    const _equipObject = paras[2]; //使用技能的军队或建筑
    const _skill = paras[3]; //技能名称
    const addParas = paras[4]; //技能附加参数对象
    const _action = paras[5];
    const skillID = addParas.skillID;
    const dj = target.getSatrap(); //获得目标的大将，并创建一个新技能给那个对象
    const newSkill = SkillAction.wLayer.skillMgr.createSkill(skillID, dj, null, true, target); //添加给新武将使用一次的军师技能
...
/// 添加buff
SkillAction.addBuff = paras => {
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
    const addParas = paras[4]; //技能附加参数对象
    const action = paras[5];
    let buffID = addParas.buffID;
    _.log('SkillAction addbuff', action.condition);
    if (buffID >= 10_000 && buffID <= 99_999) {
        //如果buffid只有5位，则补上玩家的等级作为buff等级
        buffID = buffID * 100 + USER.level;
    }
    //var buffArg = addParas['args'][0];  //buff通常只取第一个参数
    const duration = addParas.duration || skill.duration; //如果action有duration则取action的 否则取skill的

    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (SkillAction.checkAffectType(obj, equipObject, action)) {
            //如果有buffid 且每个target满足action的条件
            if (
                buffID !== undefined &&
                (!action.condition ||
                    (action.condition && SkillCondition[action.condition](obj, equiper, equipObject, skill, action)))
            ) {
                const realTarget = SkillAction.getActionTarget(obj, action);
                if (realTarget) {
...
SkillAction.skillhurt = paras => {
    ...
    if (SkillAction.checkTargetBeDebuff(target)) {
        const damage = SkillAction.calDamage(equiper, equipObject, target, skill, addParas);
...
SkillAction.beatBack = paras => {
    ...
    if (SkillAction.checkTargetBeDebuff(target)) {
        ...
        SkillAction.attackAction(damage, equipObject, target, ARMY_DEF.DAMAGE_TYPE_NORMAL);
        //target.changeTroops(-damage);
        const dx = target.x - equipObject.x;
        const dy = (target.y - equipObject.y) * GRID_WH;
        //_.log('beatBack',dx, dy);
        const dir = WORLD.getFaceXyDir(dx, dy);
        const neighbor = equipObject.pMap.getNeighbor(target.cx, target.cy, WORLD.getNeighborDirByFaceDir(dir));
        target.transferToTile(neighbor.cx, neighbor.cy);
    }
};

/// 恢复友军武将受伤状态
SkillAction.cureInjury = paras => {
    const target = paras[0]; //技能目标
    const _equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
    const _addParas = paras[4]; //技能附加参数对象
    const action = paras[5];
...
SkillAction.refresh = paras => {
    ...
    const _addParas = paras[4]; //技能附加参数对象
    const action = paras[5];
    const buffMgr = equipObject.pML.buffMgr;
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (SkillAction.checkAffectType(obj, equipObject, action) && obj.type === MAP_DEF.TYPE_UNIT) {
            const buffs = buffMgr.buffList[obj.objID];
...
SkillAction.taunt = function (paras) {
    ...
    if (this.equipObject.emitter) {
        this.equipObject.emitter.emit('taunt', { targets });
    }
};

/// 范围伤害
SkillAction.adviserAoe = paras => {
    _.log('adviserAoe');
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    ...
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (obj.force !== equiper.shiLi && SkillAction.checkTargetBeDebuff(obj)) {
            const realTarget = SkillAction.getActionTarget(obj, action);
            if (realTarget) {
                const damage = SkillAction.calAdviserDamage(equiper, equipObject, obj, skill, addParas);
...
SkillAction.aoe = paras => {
    ...
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (SkillAction.checkAffectType(obj, equipObject, action) && SkillAction.checkTargetBeDebuff(obj)) {
            const damage = SkillAction.calDamage(equiper, equipObject, obj, skill, addParas);
    ...
    if (SkillAction.checkTargetBeDebuff(target)) {
        SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
            if (obj.force === equipObject.force && typeof obj.type === MAP_DEF.TYPE_UNIT) {
                const damage = SkillAction.calDamage(equiper, obj, target, skill, addParas);
...
/// 持续性范围伤害技能
SkillAction.dotaoe = paras => {
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
    const addParas = paras[4]; //技能附加参数对象
    const interval = addParas.interval;
    //cx cy oriForce 需要先缓存下来，因为随着时间的推移可能会变化
    const cx = target.cx;
    const cy = target.cy;
    const oriForce = equipObject.force;
    const cells = SkillAction.wLayer.map.scopeCellsByTileXY(cx, cy, skill.scope, true);
    const tileNode = SkillAction.initTileNodes(cx, cy, skill.scope);
    SkillAction.wLayer.setTimeMechine(
        null,
        skill.duration * _.TIME.SEC,
        interval * _.TIME.SEC,
        () => {
            SkillAction.scopeLoop(
                cx,
                cy,
                skill.scope,
                obj => {
                    if (
                        obj.force !== oriForce &&
                        typeof obj.changeTroops === 'function' &&
                        SkillAction.checkTargetBeDebuff(obj)
                    ) {
                        const damage = SkillAction.calDamage(equiper, equipObject, obj, skill, addParas);
...
SkillAction.dot = paras => {
    ...
    const addParas = paras[4]; //技能附加参数对象
    const interval = addParas.interval;
    const funcName = addParas.function;
    const args = addParas.args;
    SkillAction.wLayer.setTimeMechine(
        null,
        skill.duration * _.TIME.SEC,
        interval * _.TIME.SEC,
        () => {
            if (SkillAction.checkTargetBeDebuff(target)) {
                const damage = target[funcName](args);
                SkillAction.attackAction(damage, equipObject, target, ARMY_DEF.DAMAGE_TYPE_NORMAL);
            }
        },
        () => {}
...
/// 使已经陷入混乱的军队延长混乱时间
SkillAction.continueHunluan = paras => {
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
...
/// 士气变化
SkillAction.changeShiQi = paras => {
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能
    const addParas = paras[4]; //技能附加参数对象
    const action = paras[5];
    const fomulaID = addParas.fomulaID;
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (SkillAction.checkAffectType(obj, equipObject, action) && !Number.isNaN(obj.shiQi)) {
            const p = [equipObject, obj, skill, equiper, addParas.isBeishui];
            const shiqi = FOMULA.run(fomulaID, p);
            if (Number.isNaN(shiqi)) {
                return;
            }
            if (shiqi > 0) {
                const frames = cc.getAnimFrames('skill/shiqi/up/%d.png');
                const upEffect = cc.createAni(
                    { anchor: [0.5, 0.5] },
                    3,
                    cc.director.getAnimationInterval() * frames.length,
                    frames
                );
                ...
                const frames = cc.getAnimFrames('skill/shiqi/down/%d.png');
                const downEffect = cc.createAni(
                    { anchor: [0.5, 0.5] },
                    3,
                    cc.director.getAnimationInterval() * frames.length,
                    frames
...
SkillAction.suckBlood = paras => {
    ...
    if (damage > 0) {
        ...
        if (equipObject.wuJiang) {
            ...
            const jwID = equipObject.wuJiang.daJiang.jueWei;
            const troopsLimit = JueWei.getTroopsLimit(jwID, equipObject.force);
...
SkillAction.cure = paras => {
    ...
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (obj.force === equipObject.force && !Number.isNaN(obj.wounded) && !Number.isNaN(obj.troops)) {
            const fomulaP = [];
            fomulaP[0] = equipObject;
            fomulaP[1] = obj;
            fomulaP[2] = skill;
            fomulaP[3] = equiper;
            const cureTroops = Math.round(FOMULA.run(fomulaID, fomulaP));
...
/// 内讧，让范围内的敌人互相攻击
SkillAction.convert = paras => {
    const target = paras[0]; //技能目标
    const _equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
    const _addParas = paras[4]; //技能附加参数对象
    const oriForceMap = {}; //存储被改变force军队的原始force
    const oriObjArr = []; //存储被改变force军队的obj
    let startIdx = 1;
    //先搜索附近是否有能够转移目标的军队
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        if (obj.force !== equipObject.force && obj.type === MAP_DEF.TYPE_UNIT) {
            ...
            oriObjArr.push(obj);
            obj.force = 1_100_000 + startIdx;
            startIdx++;
        }
    });
    for (let i = 0; i < oriObjArr.length; i++) {
        const obj = oriObjArr[i];
        const others = _(oriObjArr).without(obj);
        if (others.length > 0) {
            const idx = _.rand(others.length); //随机选一个目标
            obj.setTarget(others[idx]);
        }
    }
    SkillAction.wLayer.setTimeMechine(null, skill.duration * _.TIME.SEC, null, null, () => {
        _(oriObjArr).each((v, _k) => {
...
SkillAction.jiaobing = paras => {
    ...
    let newTarget;
    //先搜索附近是否有能够转移目标的军队
    SkillAction.scopeLoop(target.cx, target.cy, addParas.findEnemyScope, obj => {
...
SkillAction.fuse = paras => {
    ...
    //_.log('clearFire',skill.scope, target.name);
    let smallestArmy;
    let smallestTroops = 99_999_999;
    SkillAction.scopeLoop(target.cx, target.cy, skill.scope, obj => {
        _.log(obj.name);
        if (
            obj.type === MAP_DEF.TYPE_UNIT &&
            obj !== equipObject &&
            obj.force === equipObject.force &&
            obj.troops > 0 &&
            obj.troops < smallestTroops &&
            SkillAction.checkTargetBeDebuff(obj)
        ) {
...
SkillAction.attackAction = (damage, attacker, target, damageType) => {
    const info = {
        damage, //伤害
        attacker, //伤害来源
        target, //受到伤害的目标
...
//加血减血效果
SkillAction.doDamage = (target, damage, wounded) => {
    if (damage < 0) {
        //减血
        target.changeTroops(damage);
        ccs.armatureDataManager.addArmatureFileInfo('res/animation/jineng_shanghai/jineng_shanghai.ExportJson');

        const armature = new ccs.Armature('jineng_shanghai');

        //damage label
        armature.getBone('Layer3').removeDisplay(0);
        const damageNode = new cc.Node();
        const originX = 37;
        let numX = originX;
        let damageCount = Math.abs(damage);
        while (damageCount !== 0) {
            const num = damageCount % 10;
            const numSpr = cc.createSprite(`yellow_${num}.png`, {
                anchor: [0.5, 0.5]
            });
            numSpr.x = numX;
        ...
        const minusSpr = cc.createSprite('yellow_-.png', { anchor: [0.5, 0.5] });
        minusSpr.x = numX;
        ...
        armature.getAnimation().setMovementEventCallFunc((armature, _movementType, _movementID) => {
            armature.getAnimation().stop();
            armature.removeFromParent();
        });
    } else {
        //加血
        const woundedCount = wounded || -damage;
        target.changeTroops(damage);
        ...
        while (damageCount !== 0) {
            const num = damageCount % 10;
            const numSpr = cc.createSprite(`green_${num}.png`, {
                anchor: [0.5, 0.5]
            });
            numSpr.x = numX;
            damageNode.addChild(numSpr);
        ...
        const minusSpr = cc.createSprite('green_+.png', { anchor: [0.5, 0.5] });
        minusSpr.x = numX;
        damageNode.addChild(minusSpr);
        armature.getBone('Layer3').addDisplay(damageNode, 0);
...
/// 冲锋
SkillAction.charge = paras => {
    const target = paras[0]; //技能目标
    const equiper = paras[1]; //技能装备的武将
    const equipObject = paras[2]; //使用技能的军队或建筑
    const skill = paras[3]; //技能名称
    const addParas = paras[4]; //技能附加参数对象
    const path = SkillAction.wLayer.map.tilePath(equipObject.cx, equipObject.cy, target.cx, target.cy);

    equipObject.setTarget(target, path);
    const oriRate = equipObject._speedRate;
    equipObject.setSpeedRate(999, true);
    equipObject.moveFinishCB = () => {
        //_.log('moveFinishCB');
    ...
    const frames = cc.getAnimFrames('skill/charge/%d.png');
    const upEffect = cc.createAni(
        { anchor: [0.5, 0.5] },
        0,
        3 * cc.director.getAnimationInterval() * frames.length,
        frames
    );
    upEffect
        .then(() => {
    ...
    upEffect.y = 15;
    //streak
    const streak = cc.ParticleSystem.create('res/images/charge.plist');
    ...
    const cells = SkillAction.findCellsAroundPath(path);
    SkillAction.scopeLoop(
        target.cx,
        target.cy,
        skill.scope,
        obj => {
            if (
                ...
                //有几率无阵
                const prob = FOMULA.wuzhen();
                const ac = _.rand(100) / 100;
                if (ac <= prob) {
                    SkillAction.wLayer.buffMgr.addBuff(2_300_401, obj);
                }
            }
        },
        cells
...
SkillAction.rumeng = paras => {
    ...
    SkillAction.wLayer.buffMgr.addBuff(2_301_510, target, skill.duration * _.TIME.SEC);
    //_.log('兵法强度2', target.bfPower);
    SkillAction.wLayer.setTimeMechine(null, skill.duration * _.TIME.SEC, 10 * _.TIME.SEC, null, () => {
        let cureNum = Math.min(target.oriTroops * (0.11 + skill.power * 0.03), target.wounded); //使用回复参数和技能强度计算回复量, 并取该值与受伤人数中较大的一个
        const jwID = target.wuJiang.daJiang.jueWei;
        const troopsLimit = JueWei.getTroopsLimit(jwID, equipObject.force);
