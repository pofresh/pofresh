...
function dumpCPU(handle, agent, app, comd, context, param, cb) {
    if (clientHandle(handle, agent, app, comd, context, param, cb)) {
        return;
    }
    if (handle === 'monitor') {
...
function dumpMemory(handle, agent, app, comd, context, param, cb) {
    if (clientHandle(handle, agent, app, comd, context, param, cb)) {
        return;
    }
    if (handle === 'monitor') {
...
function showError(handle, agent, comd, context, cb) {
    //TODO: show error, 这是临时方案，等pofresh-monitor支持
    if (handle === 'client') {
        if (context === 'all') {
