...
function nextTimeTest(count) {
  const timer = new cronTrigger().decodeTrigger('0 0 0 1 0 2-5');
  ...
  let r1, r2;
  let start = Date.now();
  for (let i = 0; i < count; i++) r1 = cronTrigger.nextTime(value, timer);
  let end = Date.now();

  console.log('first run time : ' + (end - start));

  start = Date.now();
  for (let i = 0; i < count; i++) r2 = nextExcuteTimeTest(value, timer);
  end = Date.now();
